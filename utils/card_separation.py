...
def extract_card_crops(image, entry):
    """
    Given the original image and detected card parameters, extract oriented crops of each card.
    Args:
        image             (np.ndarray): (H, W, 3) uint8 original image.
        entry             (tuple)     : (cx, cy, angle_deg, sw, sh, sc) card parameters.
    Returns:
        crops (list): list of (h, w, 3) uint8 card crops, oriented upright.
    """
    
    cx, cy, ang, sw, sh, sc = entry
    hw = int(CARD_W * sw // 2)
    hh = int(CARD_H * sh // 2)
    ...
    a = np.deg2rad(ang)
    ca, sa = np.cos(a), np.sin(a)
    corners = ((np.array([[ca,-sa],[sa,ca]]) @ corners_loc.T).T + np.array([cx, cy])).astype(np.int32)
    min_x, min_y = corners.min(axis=0)
    max_x, max_y = corners.max(axis=0)
    crop = image[max(0, min_y):min(image.shape[0], max_y),max(0, min_x):min(image.shape[1], max_x)]
